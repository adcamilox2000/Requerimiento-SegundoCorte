...
public class CategoriaServiceImpl implements ICategoriaService{
    ...
    @Override
    public CategoriaDTORespuesta findById(Integer id) {
        CategoriaDTORespuesta categoriaRetornar = null;
        Optional<CategoriaEntity> optionalCategoria = this.servicioAccesoBaseDatos.findById(id);

        if (optionalCategoria.isPresent()) {
            CategoriaEntity categoriaEntity = optionalCategoria.get();
            categoriaRetornar = this.modelMapper.map(categoriaEntity, CategoriaDTORespuesta.class);
        }
